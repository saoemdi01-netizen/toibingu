...
import { connectDB, getAllCards, updateCardState, getAllLookups, addLookup, deleteLookup, addCard, getLightningDecks, addLightningDeck, deleteLightningDeck, getLightningCards, addLightningCards, deleteLightningCard, updateLightningCard, updateCardFields, deleteCard } from './db.js';
...
// Update general card fields (e.g. isPublished, module, etc.)
app.patch('/api/cards/:id', async (req, res) => {
  const { id } = req.params;
  const updateData = req.body;
  try {
    const updatedCard = await updateCardFields(id, updateData);
    if (!updatedCard) {
      return res.status(404).json({ error: 'Card not found' });
    }
    res.json(updatedCard);
  } catch (err) {
    res.status(500).json({ error: 'Failed to update card fields', details: err.message });
  }
});
...
// =====================================================================
// LIGHTNING DECK-BASED FLASHCARDS API
// =====================================================================

app.get('/api/decks', async (req, res) => {
  try {
    const decks = await getLightningDecks();
    res.json({ decks });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post('/api/decks', async (req, res) => {
  const { name } = req.body;
  if (!name || !name.trim()) {
    return res.status(400).json({ error: 'Tên bài học không hợp lệ.' });
  }
  try {
    const newDeck = await addLightningDeck(name);
...
app.delete('/api/decks/:deckId', async (req, res) => {
  ...
  try {
    await deleteLightningDeck(deckId);
...
app.get('/api/lightning-cards/:deckId', async (req, res) => {
  const { deckId } = req.params;
  try {
    const cards = await getLightningCards(deckId);
    res.json({ cards });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.put('/api/lightning-cards/:cardId', async (req, res) => {
  const { cardId } = req.params;
  try {
    const success = await updateLightningCard(cardId, req.body);
    if (success) {
      res.json({ success: true });
    } else {
      res.status(404).json({ error: 'Không tìm thấy card.' });
    }
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.delete('/api/lightning-cards/:cardId', async (req, res) => {
  const { cardId } = req.params;
  try {
    await deleteLightningCard(cardId);
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post('/api/cards', async (req, res) => {
  const { cards, deckId, deckName } = req.body;
  if (!cards || !Array.isArray(cards) || cards.length === 0) {
    return res.status(400).json({ error: 'Không có cards để xuất.' });
  }
  try {
    const result = await addLightningCards(cards, deckId, deckName);
    res.json(result);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
